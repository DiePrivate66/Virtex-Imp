...
- el `AuditLogAdmin` ya muestra enlaces inversos a `Limbo`, `HTML` y `JSON` cuando el `target_model` es `OfflineJournalSegment`, y enlaces a `Lote HTML` / `Lote JSON` cuando el target es `OfflineJournalSegmentBatch`
...
- ese detalle HTML batch ahora tambien lista los segmentos asociados del lote y les da salida directa a `Limbo`, `HTML` y `JSON`, para no dejar el drill-down batch desconectado del journal real
- esa misma vista HTML del lote ahora calcula en render un resumen agregado live del estado actual de sus segmentos asociados consultando el detalle offline real de cada uno, para distinguir rapido `sealed`, `open`, `footer_missing`, `integrity_error`, revision operativa y acumulado actual sin salir del lote
