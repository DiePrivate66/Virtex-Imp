...
class AnalyticsReplayTimelineTests(TestCase):
    ...
    def test_dashboard_context_includes_offline_audited_actions(self):
        audit = AuditLog.objects.create(
            ...
            target_id='sales-20260404-009',
            payload_json={
                'segment_status': 'sealed',
                'footer_present': True,
            },
            correlation_id='sales-20260404-009',
        )
    ...
    def test_dashboard_context_filters_offline_audited_actions_by_all_operational_dimensions(self):
        ...
        AuditLog.objects.create(
            ...
            event_type='offline.segment_footer_revalidated',
            target_model='OfflineJournalSegment',
            target_id='sales-20260404-026',
            payload_json={'segment_status': 'corrupt', 'footer_present': True, 'audit_result': 'footer_present'},
            correlation_id='sales-20260404-026',
        )
        AuditLog.objects.create(
            organization=self.location.organization,
            location=self.location,
        ...
        self.assertIn('sealed', context['offline_audited_action_segment_status_options'])
        self.assertIn('corrupt', context['offline_audited_action_segment_status_options'])
        self.assertTrue(
            any(
    ...
    def test_offline_bulk_runs_context_filters_by_batch_id(self):
        selected_batch = AuditLog.objects.create(
            organization=self.location.organization,
            location=self.location,
            actor_user=self.user,
            event_type='offline.segment_bulk_revalidated',
            target_model='OfflineJournalSegmentBatch',
            target_id='revalidate-batch-filter-001',
            payload_json={'processed': 5, 'succeeded': 5, 'failed': 0},
            correlation_id='revalidate-batch-filter-001',
        )
        AuditLog.objects.create(
            organization=self.location.organization,
            location=self.location,
            ...
            event_type='offline.segment_bulk_revalidated',
            target_model='OfflineJournalSegmentBatch',
            target_id='revalidate-batch-filter-XYZ',
            payload_json={'processed': 1, 'succeeded': 1, 'failed': 0},
            correlation_id='revalidate-batch-filter-XYZ',
        )

        context = build_offline_bulk_runs_context(
            periodo='semana',
            offline_bulk_batch_id='filter-001',
        )
    ...
    def test_offline_incident_batches_view_filters_by_batch_id(self):
        selected_batch = AuditLog.objects.create(
            ...
            event_type='offline.segment_bulk_review_marked',
            target_model='OfflineJournalSegmentBatch',
            target_id='review-batch-view-batchid-001',
            payload_json={'processed': 4, 'succeeded': 4, 'failed': 0},
            correlation_id='review-batch-view-batchid-001',
        )
        AuditLog.objects.create(
            organization=self.location.organization,
            location=self.location,
            ...
            event_type='offline.segment_bulk_review_marked',
            target_model='OfflineJournalSegmentBatch',
            target_id='review-batch-view-batchid-999',
            payload_json={'processed': 1, 'succeeded': 1, 'failed': 0},
            correlation_id='review-batch-view-batchid-999',
        )
        ...
        response = self.client.get(
            reverse('dashboard_offline_incident_batches'),
            {
                'offline_bulk_batch_id': 'batchid-001',
            },
        )

        self.assertEqual(response.status_code, 200)
        self.assertContains(response, 'review-batch-view-batchid-001')
        self.assertNotContains(response, 'review-batch-view-batchid-999')
        self.assertContains(response, 'value="batchid-001"')
        self.assertContains(
            response,
            f'{reverse("dashboard_offline_incident_batch_json")}?audit_log_id={selected_batch.id}',
        )
    ...
    def test_offline_incident_batch_json_accepts_batch_id(self):
        selected_batch = AuditLog.objects.create(
            ...
            event_type='offline.segment_bulk_review_marked',
            target_model='OfflineJournalSegmentBatch',
            target_id='review-batch-detail-batchid-001',
            payload_json={'processed': 2, 'succeeded': 2, 'failed': 0},
            correlation_id='review-batch-detail-batchid-001',
        )

        self.client.force_login(self.user)
        response = self.client.get(
            reverse('dashboard_offline_incident_batch_json'),
            {'batch_id': 'review-batch-detail-batchid-001'},
        )
    ...
    def test_offline_incident_batches_json_export_preserves_batch_id_filter(self):
        AuditLog.objects.create(
            ...
            event_type='offline.segment_bulk_review_marked',
            target_model='OfflineJournalSegmentBatch',
            target_id='review-batch-export-batchid-001',
            payload_json={'processed': 3, 'succeeded': 3, 'failed': 0},
            correlation_id='review-batch-export-batchid-001',
        )
        AuditLog.objects.create(
            organization=self.location.organization,
            location=self.location,
            ...
            event_type='offline.segment_bulk_review_marked',
            target_model='OfflineJournalSegmentBatch',
            target_id='review-batch-export-batchid-999',
            payload_json={'processed': 1, 'succeeded': 1, 'failed': 0},
            correlation_id='review-batch-export-batchid-999',
        )
...
class OfflineLimboDashboardTests(TestCase):
    ...
    def test_dashboard_offline_limbo_preserves_initial_segment_id_from_query(self):
        ...
        self.assertContains(response, 'Abrir HTML')
    ...
    def test_dashboard_offline_limbo_json_reports_rotation_needed_for_oversized_active_segment(self):
        with TemporaryDirectory() as temp_dir:
            ...
            snapshot_path = Path(temp_dir) / 'sales-20260404-001.snapshot.json'
            ...
            journal.append_event(
                event_id='evt-offline-rotation-1',
                payload={
                    'journal_event_type': 'sale',
                    'sale_total': '11.20',
                    'payment_status': 'PAID',
                    'payment_reference': 'OFFLINE-ROTATION-001',
                    'journal_capture_source': 'client_runtime_harness',
                    'capture_event_type': 'sale.payment_confirmed',
                    'sale_origin': 'POS',
                },
                client_transaction_id='offline-rotation-001',
                summary={'total_sales': 1, 'amount_total': '11.20'},
            )
            ...
            with override_settings(
                ...
                OFFLINE_JOURNAL_ROOT=temp_dir,
                OFFLINE_JOURNAL_STREAM_NAME='sales',
                OFFLINE_JOURNAL_SEGMENT_MAX_BYTES=1,
            ):
                response = self.client.get(reverse('dashboard_offline_limbo_json'))

        self.assertEqual(response.status_code, 200)
        payload = response.json()
        self.assertTrue(payload['rotation']['rotation_needed'])
        self.assertTrue(payload['rotation']['action_allowed'])
        self.assertEqual(payload['rotation']['segment_max_bytes'], 1)
        self.assertIn('debe rotar', payload['rotation']['reason'])
    ...
    def test_dashboard_offline_limbo_segment_detail_renders_html_payload(self):
        with TemporaryDirectory() as temp_dir:
            ...
            runtime.append_sale_event(
                event_id='evt-offline-detail-html-1',
                payload={
                    'sale_total': '9.40',
                    'payment_status': 'PAID',
                    'payment_reference': 'OFFLINE-DETAIL-HTML-001',
                    'journal_capture_source': 'server_django_sales',
                    'capture_event_type': 'sale.payment_confirmed',
                    'sale_origin': 'POS',
                },
            ...
            with override_settings(
                ...
                response = self.client.get(
                    reverse('dashboard_offline_limbo_segment_detail'),
                    {'segment_id': sealed_snapshot['segment_id']},
                )
    ...
    def test_dashboard_offline_limbo_segment_revalidate_json_records_ops_metadata(self):
        ...
        self.assertEqual(audit_log.payload_json.get('audit_result'), 'footer_present')
    ...
    def test_dashboard_offline_limbo_seal_json_seals_rotation_needed_segment(self):
        with TemporaryDirectory() as temp_dir:
            ...
            journal.append_event(
                event_id='evt-offline-manual-seal-1',
                payload={
                    'journal_event_type': 'sale',
                    'sale_total': '10.50',
                    'payment_status': 'PAID',
                    'payment_reference': 'OFFLINE-MANUAL-SEAL-001',
                    'journal_capture_source': 'client_runtime_harness',
                    'capture_event_type': 'sale.payment_confirmed',
                    'sale_origin': 'POS',
                },
                client_transaction_id='offline-manual-seal-001',
                summary={'total_sales': 1, 'amount_total': '10.50'},
            )
            ...
            with override_settings(
                ...
                response = self.client.post(reverse('dashboard_offline_limbo_seal_json'))
                recovery = recover_segment_prefix(segment_path)

        self.assertEqual(response.status_code, 200)
        payload = response.json()
        self.assertEqual(payload['action']['name'], 'seal_active_segment')
        self.assertTrue(payload['action']['performed'])
        self.assertTrue(payload['limbo']['sealed'])
        self.assertFalse(payload['rotation']['rotation_needed'])
        self.assertIn('sellado manualmente', payload['action']['detail'])
        self.assertIsNotNone(recovery.footer)
